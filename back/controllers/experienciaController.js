...
if (data.length == 0) { return res.status(500).json({ error: error.message })

}
const query =
"UPDATE experiencia SET institucion =?, cargo =?, actividades =? WHERE cliente_id =?";

conexion.query(query, [ institucion, cargo, actividades, req.params.id], (error, results) => {
if (error) {
  return res.status(500).json({ error: error.message });
}
res
  ...
  const patchExperiencia = ((req, res) => {
    ...
    const query = 'UPDATE experiencia SET estado = !estado WHERE experiencia_id = ?';
