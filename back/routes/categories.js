...
const saveCategory = (req, res) => {
  ...
  connection.query(queryCheck, [nombre], async (err, data) => {
    if (err) {
      return res.status(500).json({
        success: false,
    ...
    try {
      ...
      connection.query(query, [imgPath, nombre, montoInvMin, montoInvMax, porcentaje_interes], (err) => {
        if (err) {
          res.status(500).json({
            success: false,
...
const updateImgCategory = (req, res) => {
  ...
  if (!req.files || Object.keys(req.files).length === 0 || !req.files.image) {
    ...
    connection.query(query, [nombre, monto_minimo_inversion,monto_maximo_inversion, porcentaje_interes, req.params.id], (err) => {
      if (err) {
        res.status(500).json({
    ...
    connection.query(querySelect, [req.params.id], async (err, data) => {
      ...
      try {
        ...
        connection.query(queryUpdate, [imgPath, nombre,monto_minimo_inversion,monto_maximo_inversion,  req.params.id], (err) => {
          if (err) {
...
const addRubr = async (req, res) => {
  ...
  connection.query(query, [categoria_persona_id, usuario_id], (error, results) => {
    if (error) {
