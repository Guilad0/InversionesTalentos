...
const uploadimageUserCloudinary = (req, res) => {
    ...
    connection.query(query, [req.params.id], async (err, results) => {
        ...
        try {
            ...
            connection.query(query, [secure_url, req.params.id], (err) => {
                if (err) {
...
const putInvestors = async (req, res) => {
    ...
    connection.query( query, [correo, req.params.id], (err, results) =>{
        if( err ){
...
const addInfinversionista = (req, res) => {
    ...
    let query = "select * from usuarios where usuario_id=?";
    connection.query(query, [id_inversionista], (err, results) => {
        if (err) {
        res.status(500).json({
            err,
        });
        ...
        connection.query(query, [id_inversionista], (err, data) => {
        if (err) {
            res.status(500).json({
            err,
            });
...
const changeRolUser = (req,res)=>{
    ...
    connection.query(query,[usuario_id],(err,results) =>{
        ...
        connection.query(query,[rol,usuario_id], (err,results) => {
            if( err ){
                res.status(500).json({
                    err,
