...
const getContact = (req, res) => {
    ...
    connection.query(sql, [`%${search}%`, `%${search}%`, `%${search}%`, limit, offset], (error, results) => {
        if (error) {
            res.status(500).json({ error });
        } else {
            connection.query('SELECT COUNT(*) AS total FROM contacto WHERE nombre LIKE ? OR apellido LIKE ? OR email LIKE ?', [`%${search}%`, `%${search}%`, `%${search}%`], (countError, countResults) => {
                if (countError) {
    ...
    connection.query(sql, values, (error, results) => {
        if (error) {
            ...
            if (results.length === 0) {
                ...
                connection.query(sql2, values, (error) => {
                    if (error) {
                        res.status(500).json({
                            message: 'Error al realizar la petición',
                            error: error.message
...
const updateResponse = async (req, res) => {
    ...
    connection.query(sqlSelect, [id], (error, results) => {
        ...
        connection.query(sqlUpdate, [respuesta, id], async (updateError) => {
            if (updateError) {
                return res.status(500).json({
