...
const postInformacion = (req, res) => {
  ...
  connection.query(query, [cliente_id, ocupacion, descripcion, monto_inversion, cantidad_maxima_inversiones, preparacion, estudios, vision], (error, results) => {
    if (error) {
      return res.status(500).json({ error: error.message });
    }
    res
...
const saveVideo = (req, res) => {
  ...
  connection.query(query, [cliente_id], async (err, data) => {
    if (err) {
    ...
    try {
      ...
      connection.query(query, [videoPath, cliente_id], (err) => {
        if (err) {
...
const getClienteIdByUsuarioId = (req, res) => {
  ...
  let query = `SELECT i.cliente_id
              ...
              WHERE u.usuario_id = ?;`;
  connection.query(query, [usuario_id], (err, data) => {
    if (err) {
