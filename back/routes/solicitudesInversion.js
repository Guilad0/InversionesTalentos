...
const solicitudesInversionRouter = express.Router();
...
const getInversoresDeSolicitud = (req, res) => {
  ...
  connection.query(query, (err, resultsInversores) => {
    if (err) {
      return res
        .status(500)
        .json({ msg: "Error al obtener la solicitud de inversión", err });
...
const finalizarInversion = (req, res) => {
  ...
  connection.query(query, [fechaActual, req.params.id], (err, results) => {
    if (err) {
        ...
        connection.query(query, (err, results) => {
          ...
          const promises = usersId.map((cliente) => {
            ...
            return new Promise((resolve, reject) => {
              connection.query(query, [cliente.tipoMovimiento, cliente.id], (err, results) => {
                if (err) {
...
const createSolicitudInversion = (req, res) => {
  ...
  connection.query(updateQuery, [cliente_id], (updateErr) => {
    if (updateErr) {
      return res
        .status(500)
    ...
    connection.query(queryCategoria, [cliente_id], (err, categoriaResults) => {
      if (err) {
        return res
          .status(500)
...
const procesarSolicitudInversion = (req, res) => {
  ...
  connection.query(query, [id], (err, results) => {
    ...
    connection.query(query, [minInv, idCliente], (err, results) => {
      if (err) {
        return res
          .status(500)
          .json({
...
const getSolicitudesInversionByUserId = (req, res) => {
  ...
  connection.query(countQuery, [userId], (countError, countResults) => {
    if (countError) {
...
solicitudesInversionRouter.get("/", getSolicitudesInversion);
solicitudesInversionRouter.get("/aprobados", getSolicitudesInversionAprobados);
solicitudesInversionRouter.get("/pendientes", getSolicitudesInversionPendientes);
solicitudesInversionRouter.get("/rechazados", getSolicitudesInversionRechazados);
solicitudesInversionRouter.get("/:id", getSolicitudInversionById);
solicitudesInversionRouter.post("/", createSolicitudInversion);
solicitudesInversionRouter.put("/:id", updateSolicitudInversion);
solicitudesInversionRouter.put("/procesarInversionByIUser/:id", procesarInversionByUser);
solicitudesInversionRouter.put("/aprobar/:id", procesarSolicitudInversion);
solicitudesInversionRouter.delete("/:id", deleteSolicitudInversion);
solicitudesInversionRouter.get("/user/:userId", getSolicitudesInversionByUserId);
solicitudesInversionRouter.get("/showButton/:id", showButton);
solicitudesInversionRouter.get("/getTotals/totals", getTotals),
solicitudesInversionRouter.get("/getInversoresDeSolicitud/:id", getInversoresDeSolicitud),
solicitudesInversionRouter.put("/finalizarInversion/:id", finalizarInversion),
solicitudesInversionRouter.put("/revertirInversion/:id", revertirInversion),
solicitudesInversionRouter.put("/cambiarEstadoProceso/:id", cambiarEstadoProceso);
solicitudesInversionRouter.get("/getSolicitudByClienteId/:id", getSolicitudByClienteId);


module.exports = {
  solicitudesInversionRouter, 
