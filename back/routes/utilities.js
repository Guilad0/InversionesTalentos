...
const { response: res, request: req, query, json } = express;
...
const uploadimageUserCloudinaryHome = async (req, res) => {
  ...
  try {
    ...
    connection.query(query, [secure_url], (err) => {
      if (err) {
...
const putTextHome = (req, res) => {
  ...
  connection.query(query, [req.params.text], (err, results) => {
    if (err) {
      res.status(500).json({
        err,
...
const savePercentajerUser = (req, res) =>{
  ...
  connection.query(query, [req.query.porcentaje,req.params.id], (err, results) =>{
    if (err) {
      res.status(500).json({
        err,
        msg: "error al guardar el texto",
...
const changeNameSystem = (req, res) =>{
  ...
  connection.query(query,[ req.query.nombre    ],(err) =>{
    if(err){
      res.status(500).json({
        err
      })
